...
function martanian_script_enqueuer()
{
    ...
    wp_enqueue_style('theme-main', get_stylesheet_directory_uri().'/_assets/less/main.less');
}
...
class martanian_most_commented_posts extends WP_Widget
{
    ...
    public function form($instance)
    {
        ...
        <p>
            ...
            <input class="widefat" name="<?php echo $this->get_field_name('posts'); ?>" type="text"
...
function martanian_shortcode_video($attr)
{
    ...
    if (!isset($attr['from']) || $attr['from'] == '' || !in_array($attr['from'], array('youtube', 'vimeo')) || !isset($attr['id']) || $attr['id'] == '') return false;
        ...
        switch ($attr['from']) {
            ...
            case 'youtube':
                $src = 'http://www.youtube.com/embed/' . $attr['id'] . '?vq=hd1080;rel=0;showinfo=0';
                break;
            case 'vimeo':
                $src = 'http://player.vimeo.com/video/' . $attr['id'] . '?autoplay=&amp;title=0&amp;byline=0&amp;portrait=0';
                break;
        }
